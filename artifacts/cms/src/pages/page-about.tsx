...
const DEFAULT_SECTIONS: AboutSection[] = [
  ...
  { id: "how-it-works", type: "steps", enabled: true, heading: "How The Tribunal Works", steps: [ { num: "01", title: "Choose a question.", body: "Debates ask what people believe. Predictions ask what people think will happen." }, { num: "02", title: "Vote privately.", body: "Your name and email are not shown with your vote." }, { num: "03", title: "See the result.", body: "Results are shown publicly in aggregate, with country and topic breakdowns where enough data exists." }, { num: "04", title: "Save your activity.", body: "You can vote without an account. If you sign up, you can view previous votes, track predictions and continue from another device." }, { num: "05", title: "Go deeper.", body: "Pulse adds sourced public signals. Voices profiles people with a view worth recording. The Majlis creates a private room for serious conversation." }, { num: "06", title: "Trust the process.", body: "Questions are human reviewed. Results are opinion signals, not scientific polling. No bots. No sponsored sentiment. No fake activity." } ] },
      ...
      {config.sections.map((section, i) => (
        <section key={section.id} className={`border rounded-sm p-4 space-y-3 ${section.enabled === false ? "border-border/40 opacity-60" : "border-border"}`}>
          ...
          {section.type === "cta" && (<>
            ...
            <p className="text-xs text-muted-foreground">Pulse / Voices / Majlis buttons appear automatically when enabled in Site Settings.</p>
          </>)}
        </section>
      ))}
